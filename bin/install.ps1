...
if ($julia_version -ge [Version]"0.7") {
    if (Test-Path "Project.toml") {
        ...
        $env:JL_SUCCESS_SCRIPT = "using Pkg; Pkg.add(\`"Coverage\`"); using Coverage; Codecov.submit(process_folder())"
    } else {
        ...
        $env:JL_SUCCESS_SCRIPT = "using Pkg; cd(Pkg.dir(\`"$projectname\`")); Pkg.add(\`"Coverage\`"); using Coverage; Codecov.submit(process_folder())"
    }
} else {
    $projectname = $env:APPVEYOR_PROJECT_NAME -replace '\.jl$',''    
    $env:JL_BUILD_SCRIPT = "Pkg.clone(pwd(), \`"$projectname\`"); Pkg.build(\`"$projectname\`")"
    $env:JL_TEST_SCRIPT = "Pkg.test(\`"$projectname\`", coverage=true)"
    $env:JL_SUCCESS_SCRIPT = "cd(Pkg.dir(\`"$projectname\`")); Pkg.add(\`"Coverage\`"); using Coverage; Codecov.submit(process_folder())"
}
